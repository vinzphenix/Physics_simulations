...
f5 = lambda xi, eta: 1.5 * (exp(-3 * (xi - L / 4) ** 2 - 3 * (eta - H / 4) ** 2) + exp(
    - 3 * (xi - 3 * L / 4) ** 2 - 3 * (eta - 3 * H / 4) ** 2)) * sin(pi * xi / L) * sin(pi * eta / H)  # 6-7


def update_plot(idx):
    ax.view_init(elev=20 - 8 * idx / nt, azim=20 + 60 * idx / nt)
...
if __name__ == "__main__":
    ...
    Tend = 5.  # duration of the simulation
    ...
    animate = FuncAnimation(fig, update_plot, nt, interval=20, repeat_delay=3000, repeat=True)
    #animate.save('rectangular_membrane_7.html', fps=fps)
    plt.show()
