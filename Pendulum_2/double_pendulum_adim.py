...
def path_double_pendulum(sim, time_series, wrap=False):
    ...
    if wrap:
        ...
        for k in range(len(time_series)):
            time_series[k] = np.insert(time_series[k], idxs, np.nan)
        idxs = np.where(np.abs(np.diff(time_series[3])) > 3)[0] + 1
        for k in range(len(time_series)):
            time_series[k] = np.insert(time_series[k], idxs, np.nan)
...
def load_configuration(i):
    ...
    if i == 1:
        ...
        phi1_0, om1_0, phi2_0, om2_0 = u_zero_list[i - 5]
        ...
        phi1_0, om1_0, phi2_0, om2_0 = u_zero_list[i - 11]
        ...
        phi1_0, om1_0, phi2_0, om2_0 = u_zero_list[i - 16]
        ...
        phi1_0, om1_0, phi2_0, om2_0 = u_zero_list[i - 21]
        ...
        phi1_0, om1_0, phi2_0, om2_0 = u_zero_list[i - 25]
        ...
        phi1_0, om1_0, phi2_0, om2_0 = u_zero_list[i - 31]
        ...
        # phi1_0, om1_0, phi2_0, om2_0 = 0.00000, 1.24501, -1.25257, -1.01287  # bit more fuzzy
...
if __name__ == "__main__":
    ...
    setup = {
        't_sim':60.0, 'fps': 30, 'slowdown': 0.25, 'oversample': 10
    }

    params, initials = load_configuration(20)
