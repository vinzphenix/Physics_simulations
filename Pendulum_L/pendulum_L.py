# ! not working correctly ! equations of motion probably not correct
...
def see_animation(save="", arrow=False, phaseSpace=0):
    ...
    def update(i):
        ...
        rect1.set(xy=(Ax[i], Ay[i]), angle=45. + degrees(th[i]))
        rect2.set(xy=(Bx[i], By[i]), angle=-45. + degrees(th[i]))
        rect3.set(xy=(Cx[i], Cy[i]), angle=-90. + degrees(phi1[i]))
        rect4.set(xy=(Dx[i], Dy[i]), angle=-90. + degrees(phi2[i]))
